...
class MGCTrainer:
    ...
    def run(self):
        ...
        for epoch in range(1, self.args['epoch'] + 1):
            ...
            # validation
            if epoch % 1 == 0:
                acc, plot, loss = self.test(self.val_loader)
                print(f'{epoch} val_acc: {acc}')
                self.logger.log_metric('val/accuracy', acc, epoch)
                lr = self.optimizer.param_groups[0]['lr']
                self.logger.log_metric('lr/epoch', lr, epoch)
                self.logger.log_metric('val_loss/epoch', loss, epoch)
        ...
        acc, plot, loss = self.test(self.eval_loader)
    ...
    def test(self, loader):
        ...
        with torch.set_grad_enabled(False):
            for x, label in tqdm(loader, desc='test', ncols=90):                
                ...
                # feed forward
                prediction, loss = self.model(x, label_cuda, train=False)
                loss_sum += loss.item()
            
                # count
                label = label.item()
                prediction = torch.max(prediction, dim=1)[1].item()

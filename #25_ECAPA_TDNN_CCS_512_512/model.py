...
class ECAPA_TDNN(nn.Module):
    def __init__(self, args):
        ...
        self.layer1 = Bottle2neck(args['C'], args['C'] + 512, 3, 2, 8)
        self.layer2 = Bottle2neck(args['C'], args['C'] + 512, 3, 3, 8)
        self.layer3 = Bottle2neck(args['C'], args['C'] + 512, 3, 4, 8)
        self.layer4 = nn.Conv1d(3 * args['C'], 756, kernel_size=1)

        self.attention = nn.Sequential(
            nn.Conv1d(6876, 256, kernel_size=1),
            nn.ReLU(),
            nn.BatchNorm1d(256),
            nn.Tanh(),
            nn.Conv1d(256, 2292, kernel_size=1),
            nn.Softmax(dim=2)
        )
        self.bn5 = nn.BatchNorm1d(4584)
        self.fc6 = nn.Linear(4584, 28)
    ...
    def forward(self, x, labels=None):
        ...
        x1 = self.layer1(x)
        x1, x1_out = x1[:, :-512, :], x1[:, -512:, :]
        x2 = self.layer2(x + x1)
        x2, x2_out = x2[:, :-512, :], x2[:, -512:, :]
        x3 = self.layer3(x + x1 + x2)
        x3, x3_out = x3[:, :-512, :], x3[:, -512:, :]

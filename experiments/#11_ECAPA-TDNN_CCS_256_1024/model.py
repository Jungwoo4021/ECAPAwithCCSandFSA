...
class ECAPA_TDNN(nn.Module):
    def __init__(self, args):
        ...
        self.mid_band_extractor = FeatureExtractor(48, 1024, 256)

        self.attention = nn.Sequential(
            nn.Conv1d(8064, 256, kernel_size=1),
            nn.ReLU(),
            nn.BatchNorm1d(256),
            nn.Tanh(),
            nn.Conv1d(256, 2688, kernel_size=1),
            nn.Softmax(dim=2)
        )
        self.bn5 = nn.BatchNorm1d(5376)
        self.fc6 = nn.Linear(5376, 10)
...
class FeatureExtractor(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer4(torch.cat((x1, x2, x3),dim=1))  # 1920
        x = self.relu(x)

        x = torch.cat((x1_out, x2_out, x3_out, x), dim=1)   # 1920 + 256*3 = 2688

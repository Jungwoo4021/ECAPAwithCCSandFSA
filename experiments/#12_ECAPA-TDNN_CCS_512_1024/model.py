...
class ECAPA_TDNN(nn.Module):
    def __init__(self, args):
        ...
        self.mid_band_extractor = FeatureExtractor(48, 1024, 512)

        self.attention = nn.Sequential(
            nn.Conv1d(11520, 256, kernel_size=1),
            nn.ReLU(),
            nn.BatchNorm1d(256),
            nn.Tanh(),
            nn.Conv1d(256, 3840, kernel_size=1),
            nn.Softmax(dim=2)
        )
        self.bn5 = nn.BatchNorm1d(7680)
        self.fc6 = nn.Linear(7680, 10)
...
class FeatureExtractor(nn.Module):
    def __init__(self, in_channel, C, M):
        ...
        self.layer4 = nn.Conv1d(3 * C, int(1.5*(C+M)), kernel_size=1)   # 채널을 반으로 줄임
    ...
    def forward(self, x):
        ...
        x = self.layer4(torch.cat((x1, x2, x3),dim=1))  # 2304
        x = self.relu(x)

        x = torch.cat((x1_out, x2_out, x3_out, x), dim=1)   # 2304 + 512*3 = 3840

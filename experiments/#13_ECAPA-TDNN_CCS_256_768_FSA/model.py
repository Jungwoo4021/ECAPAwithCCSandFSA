...
class ECAPA_TDNN(nn.Module):
    def __init__(self, args):
        ...
        self.layer4 = Bottle2neck(2304, 1152, 1, 1, 8)
        self.relu = nn.ReLU()

        self.attention = nn.Sequential(
            nn.Conv1d(5760, 256, kernel_size=1),
            nn.ReLU(),
            nn.BatchNorm1d(256),
            nn.Tanh(),
            nn.Conv1d(256, 1920, kernel_size=1),
            nn.Softmax(dim=2)
        )
        self.bn5 = nn.BatchNorm1d(3840)
        self.fc6 = nn.Linear(3840, 10)
    ...
    def forward(self, x, labels=None):
        ...
        x4, x4_out = self.band4(x[:, 30:48, :]) # 576 / 192
        x = torch.cat((x1, x2, x3, x4), dim=1) # 2304

        x = self.layer4(x) # 1152

        x = torch.cat((x, x1_out, x2_out, x3_out, x4_out), dim=1) # 1920
        ...
        gx = torch.cat((x, temp1, temp2), dim=1) # 5760
...
class FeatureExtractor(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat((x1, x2, x3), dim=1) # 576
        x_out = torch.cat((x1_out, x2_out, x3_out), dim=1)  # 192

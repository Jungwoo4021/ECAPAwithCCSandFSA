...
class ECAPA_TDNN(nn.Module):
    def __init__(self, args):
        ...
        self.mid_band_extractor = FeatureExtractor(48, 512, 512)

        self.attention = nn.Sequential(
            nn.Conv1d(9216, 256, kernel_size=1),
            nn.ReLU(),
            nn.BatchNorm1d(256),
            nn.Tanh(),
            nn.Conv1d(256, 3072, kernel_size=1),
            nn.Softmax(dim=2)
        )
        self.bn5 = nn.BatchNorm1d(6144)
        self.fc6 = nn.Linear(6144, 10)
...
class FeatureExtractor(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer4(torch.cat((x1, x2, x3),dim=1))  # 1536
        x = self.relu(x)

        x = torch.cat((x1_out, x2_out, x3_out, x), dim=1)   # 1536 + 512*3 = 3072
